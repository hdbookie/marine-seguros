#!/usr/bin/env python3
"""Debug why Manutenção Administrativa isn't detected as parent"""
...
for idx, row in df.iterrows():
    if pd.notna(row.iloc[0]) and str(row.iloc[0]).strip() == target_item:
        ...
        # Look for expected children
        expected_children = ['Material de consumo', 'Material de limpeza', 'Limpeza', 'Material de expediente']
        
        print(f"\nLooking for expected children:")
        found_sum = 0
        
        for i in range(1, min(10, len(df) - idx)):
            child_idx = idx + i
            ...
            if child_label in expected_children:
                ...
                child_value = extractor._get_numeric_value(df.iloc[child_idx], annual_col)
                if child_value > current_value * 0.5:  # Stop if we hit a large item
                    print(f"  🛑 Stopped at '{child_label}' (R$ {child_value:,.2f}) - too large")
                    break
                elif child_value > 0:
