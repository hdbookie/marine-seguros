#!/usr/bin/env python3
"""Debug specific parent detection"""
...
# Find "Tecnologia e Comunicação"
target_item = "Tecnologia e Comunicação"
target_idx = None
...
if target_idx:
    ...
    for i in range(1, min(11, len(df) - target_idx)):
        ...
        # Check early break condition
        if len(potential_children) >= 2 and next_value > children_sum * 0.8:
            print(f"  Row {next_idx}: '{next_label}' (R$ {next_value:,.2f}) - STOPPED (likely new parent)")
            break
        
        if next_value > 0:
            potential_children.append((next_label, next_value))
            children_sum += next_value
            print(f"  Row {next_idx}: '{next_label}' (R$ {next_value:,.2f}) - ADDED as child")
        else:
            print(f"  Row {next_idx}: '{next_label}' (R$ {next_value:,.2f}) - SKIPPED (zero value)")
    
    print(f"\nPotential children found: {len(potential_children)}")
    for child_name, child_value in potential_children:
        print(f"  - {child_name}: R$ {child_value:,.2f}")
    
    print(f"\nChildren sum: R$ {children_sum:,.2f}")
