...
class NonOperationalCostExtractor:
    ...
    def extract(self, df: pd.DataFrame) -> Dict:
        ...
        # First, look for the main "CUSTOS NÃO OPERACIONAIS" row
        ...
        for idx, row in df.iterrows():
            ...
            # Check if this is the main CUSTOS NÃO OPERACIONAIS row
            if label.upper().strip() == "CUSTOS NÃO OPERACIONAIS":
                ...
                # Use only this value as the total
                data["annual"] = row_data["annual"]
                data["monthly"] = row_data["monthly"].copy()
                main_row_found = True
                break
        
        # If we didn't find the main row, fall back to individual items
        if not main_row_found:
            for idx, row in df.iterrows():
                label = self._get_row_label(df, idx)
                if not label:
                    continue

                if any(re.search(pattern, label, re.IGNORECASE) for pattern in self.patterns[1:]):  # Skip first pattern
                    row_data = self._extract_row_data(df, idx)
                    item_key = self._normalize_key(label)
                    data["line_items"][item_key] = {
                        "label": label,
