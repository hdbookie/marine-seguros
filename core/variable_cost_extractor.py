...
class VariableCostExtractor:
    ...
    def extract_costs(self, df: pd.DataFrame, year: int) -> Dict:
        extracted_costs = {
            'line_items': {},
            'categories': {}
        }
        ...
        for idx, row in df.iterrows():
            ...
            is_subtotal_item = False
            annual_value = 0
            monthly_values = {}

            # Attempt to extract values first to pass to _is_subtotal
            ...
            is_subtotal_item = self._is_subtotal(row_label, annual_value)

            if category and category != 'revenue' and not is_subtotal_item: # Only process cost/expense categories that are not subtotals
    ...
    def _classify_line_item(self, label: str) -> str:
        ...
        for category, patterns in self.cost_patterns.items():
            for pattern in patterns:
                if pattern in label_upper:
                    return category
        return None # Not a recognized cost/expense
    ...
    def _is_subtotal(self, label: str, value: float) -> bool:
        ...
        # Heuristic: if value is exactly 0 and it's not a known variable cost line, it might be a subtotal
        if value == 0 and not any(pattern in label_upper for pattern in self.cost_patterns['variable_costs']):
            return True
